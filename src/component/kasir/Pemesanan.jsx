   ...
   const  [id_menu,setIdMenu]=useState("")
    ...
    function handleIncreaseClick(id_menu) {
      ...
      const handleSubmit = async (e) => {
        ...
        try {
          await axios.post("http://localhost:8080/transaksi/add", data_transaksi, {
            headers,
          });
          await axios.put(
            "http://localhost:8080/meja/" + selectedOption.id,
            updatedStatusMeja,
            { headers }
          );
          navigate("/riwayat");
    ...
    return (
        <div>
            <div className='fixed top-0 w-full pointer-events-none'>
                <select onChange={handleSelectChange} className="pointer-events-auto text-sm rounded-lg block p-2.5 bg-green-700 my-24 mx-8 float-right py-3 placeholder-gray-400 text-white focus:ring-sky-900 focus:border-sky-900">
            ...
            <div>
                ...
                <div className="flex flex-wrap gap-5 mt-5 ml-5 bg">
                    {menu.map((menu) => {
                        if (menu.jenis === 'makanan') {
                            return (
                                <div key={menu.id} className="max-w-sm bg-green-500 border border-gray-200 rounded-lg shadow">
                ...
                <div className="flex flex-wrap gap-5 mt-5 ml-5">
                    {menu.map((menu) => {
                        if (menu.jenis === 'minuman') {
                            return (
                                <div key={menu.id}
                                    className="max-w-sm bg-green-500 border border-gray-200 rounded-lg shadow">
                                    <img className="w-80 h-60 p-8 rounded-t-lg" src={`http://localhost:8080/foto/${menu.gambar}`} alt="product" />
                                    <div className="px-5 pb-5">
                                        <h5 className="text-xl font-semibold tracking-tight text-gray-900">{menu.nama_menu}</h5>
                                        <div className="mt-2 flex items-center justify-between">
            ...
            <div className='fixed bottom-0 w-full pointer-events-none'>
                <button onClick={checkPemesanan} className='pointer-events-auto bottom-0 my-8 mx-8 float-right inline-block px-10 py-3 rounded-lg bg-green-700 text-white'>Pesan</button>
            </div>
                ...
                <div>
                    <div className="justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none">
                        <div className="relative w-auto my-6 mx-auto max-w-3xl max-h-screen">
                            <div className="flex h-full flex-col overflow-y-auto bg-white shadow-xl rounded-xl">
                                <div className="flex-1 overflow-y-auto py-6 px-4 sm:px-6">
                                    <div className="flex items-start justify-between">
                                        <p className="text-lg font-medium text-gray-900">Pesanan Meja no.{selectedMeja}</p>
                                ...
                                <form onSubmit={handleSubmit} className="border-t border-gray-200 py-6 px-4 sm:px-6">
                                    ...
                                    <div className="mt-6">
                                        <button type="submit" className="w-full flex items-center justify-center rounded-md border border-transparent bg-green-700 px-6 py-3 text-base font-medium text-white shadow-sm hover:bg-green-800">
                                            Checkout
                                        </button>
                                    </div>
                                    <div className="mt-6 flex justify-center text-center text-sm text-gray-500">
                                        <button type="button" className="font-medium text-green-700 hover:text-green-800" onClick={() => setShowModal(false)}>
                                            Continue Shopping
