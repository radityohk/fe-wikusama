...
export default function Riwayat() {
    ...
    const handleBayar = async (id) => {
        ...
        try {
            await axios.put("http://localhost:8080/transaksi/" + id, updatedStatusTransaksi, {headers});
                                ...
                                <td className="px-6 py-4">
                                    {transaksi.status === "belum bayar" ? (
                                        <div className="flex gap-2">
                                            ...
                                            <button onClick={() => handleBayar(transaksi.id)} className="inline-flex items-center gap-1 rounded-full bg-indigo-50 px-2 py-1 text-xs font-semibold text-rose-600">
                                                Bayar
                                            </button>
                                        </div>
                                    ) : (
